...
export default function ApplyModal({ internship, onClose, onApplied }: Props) {
  ...
  function openEmailDraft() {
    ...
    window.open(`https://mail.google.com/mail/?view=cm&fs=1&to=${to}&su=${subj}&body=${body}`, '_blank', 'noopener,noreferrer');
  }
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {tab === 'overview' ? (
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="mt-4 p-3 rounded-lg bg-slate-800 flex items-center justify-between">
                  ...
                  <span className={`text-2xl font-black ${
                    ...
                    internship.composite_score >= 65 ? 'text-amber-400' : 'text-orange-400'
              ...
              {/* Why good fit */}
              <div className="p-4 rounded-lg bg-amber-900/30 border border-amber-700/40">
                <h3 className="text-sm font-semibold text-amber-300 mb-2">Why You're a Strong Fit</h3>
                <p className="text-sm text-amber-200/80 leading-relaxed">{internship.why_good_fit}</p>
              </div>

              {internship.relocation_provided && (
                <div className="flex items-center gap-2 text-sm text-emerald-400">
                  <CheckCircle size={14} />
                  Relocation assistance / housing provided
                </div>
              )}
            </div>
          ) : (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">Tailored to your resume and this specific role</p>
                </div>
                <button
                  onClick={copyLetter}
              ...
              <pre className="text-xs text-slate-300 bg-slate-800 rounded-lg p-4 whitespace-pre-wrap leading-relaxed font-mono border border-slate-700">
                {coverLetter}
              </pre>
            </div>
          )}
