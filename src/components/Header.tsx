      ...
      <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="hidden sm:block">
            <span className="font-bold text-white">InternTrack</span>
            <span className="text-slate-500 text-xs ml-2">Ammer Boorenie · TAMU MSE '27</span>
          </div>
        </div>
          ...
          {NAV.map(({ id, label, icon: Icon }) => (
            ...
            >
              ...
              {id === 'tracker' && appliedCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-orange-500 text-slate-950 text-xs font-black rounded-full w-4 h-4 flex items-center justify-center">
                  {appliedCount}
                </span>
              )}
            </button>
